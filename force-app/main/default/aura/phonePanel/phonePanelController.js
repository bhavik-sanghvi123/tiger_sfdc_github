...
({
    ...
    init: function(cmp, event, helper) {
        ...
        console.log("---REached phonepanel TELE----")
        cmp.set('v.searchResults', []);
        helper.handleOutgoingCalls(cmp);
        helper.loginCheck(cmp);
		helper.NotificationCheck(cmp);
        console.log(cmp.get('v.presence'));
        console.log("---Current return is----");
        console.log(cmp.get('v.returnAvailable'));
//                if (cmp.get('v.returnAvailable') == "true")
//                {
//                    console.log("---Return Triggered----");
//                    cmp.set('v.presence', "Available");
...
//                {
//                    console.log("avilable presence Detected, detecting follow up....");

//                    if (cmp.get('v.totalFollowUp') != 0)
//                    {
//                        cmp.set('v.presence', "Paused");
//                        console.log("Follow up Detected, stopping agent....");
//                        helper.updatePresence(cmp, event, helper);
//        helper.getTimeSheet(cmp, event, helper,"Paused");
//        helper.changeStatusVicidial(cmp, event,helper,"Paused");
//        }
//            else {
//             console.log("returncall Detected, calling helper agent....");
//             helper.updatePresence(cmp, event, helper);
//        helper.getTimeSheet(cmp, event, helper,cmp.get('v.presence'));
//             helper.changeStatusVicidial(cmp, event, helper,cmp.get('v.presence'));
...
//}
        ...
        test: function(cmp, event, helper) {
            ...
            var myquery = "SELECT Id,CreatedDate,CaseNumber FROM Case WHERE CreatedById = '0050w000004O0pSAAS' ORDER BY CreatedDate desc Limit 1";
             var newcaseid =0;   
            var queryResult = Database.query(queryresult);
            queryResult = response.getReturnValue();
    ...
    onOnlinePresenceChanged: function (cmp, event, helper) {
        if(cmp.get('v.presence')=="Paused"){
            
          cmp.set('v.message', "Call from tab...");
	...
	const Http = new XMLHttpRequest();
    let url = 'https://apps.tdcx.com.my/SalesforceTest/api/values/301/10001-User@1234-2ndAt-2023-02-10-17-17-17'
    console.log(url);      
    Http.open("GET", url);
    Http.send();
       
    },
        selectPauseCode: function (cmp, event, helper) {
         var e = document.getElementById("Pause_Codes");
        var value = e.value;
        var text = e.options[e.selectedIndex].text;
                        cmp.set('v.pausecode',"withcode");
